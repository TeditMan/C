...
void delete_idx(List *&list, unsigned idx) {
    unsigned new_length = length(list) - 1;
    int *array_tmp = new int[new_length], *array = get_array_of_list(list);
    for (int i = 0; i < idx; i++) {
        array_tmp[i] = array[i];
    }
    for (unsigned i = idx; i < new_length; i++) {
        array_tmp[i] = array[i + 1];
    }
    list = createList(array_tmp, new_length);
}

int get_idx(List *list, unsigned idx) {
    Node *tmp = list -> head;
