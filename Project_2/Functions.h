...
void create_config(int (&A)[n][3], int width, int height, PRNG generator) {
    for (auto & i : A) {
...
void do_movement(int (&A)[n][3], int a_four[4][2], PRNG generator, int (&a_o_t_l_s)[n][2]) {
    for (int i = 0; i < n; i++) {
        if (A[i][2] == 1) {
            int var_int = random(generator, 1, 4);
